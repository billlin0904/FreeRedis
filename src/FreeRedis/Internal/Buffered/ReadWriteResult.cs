...
{
    internal class ReadWriteResult : AsyncResult
    {
        ...
        public ReadWriteResult(AsyncCallback callback, object state) 
            : this(callback, state, null, 0, 0)
        {
        }

        public ReadWriteResult(AsyncCallback callback, object state, byte[] buffer, int offset, int count)
            : base(callback, state)
        {
            Buffer = buffer;
            Offset = offset;
